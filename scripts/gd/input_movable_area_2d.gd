...
func movement_and_comprovements(movement_direction:Vector2):
	...
	if far_area.get_overlapping_bodies().filter(func (c): return c is CharacterBody2D): return
	...
	if near_area.has_overlapping_areas(): 
		for area in near_area.get_overlapping_areas():
			...
			if far_area.has_overlapping_bodies() or far_area.has_overlapping_areas(): return
